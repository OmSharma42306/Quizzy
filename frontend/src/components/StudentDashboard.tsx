...
export default function StudentDashboard() {
    ...
    const handleSubmit = () => {
        if (selectedOption === null || disableSubmitButton) return;
        ...
        console.log("Submitted:", options[selectedOption]);
        setDisableSubmitButton(true);
        setShowToast(true);
    ...
    const getTimerDots = () => {
        ...
        for (let i = 0; i < totalDots; i++) {
            ...
            dots.push(
                <div
                    key={i}
                    className={`w-3 h-3 rounded-full ${color} transition-colors duration-300`}
                ></div>
            );
        }
        return dots;
    ...
    return (
        <div className="relative min-h-screen bg-gradient-to-b from-purple-900 via-purple-800 to-purple-900 flex flex-col items-center justify-center p-4">

            {/* ✅ Toast */}
            {showToast && (
                <div className="absolute top-8 right-8 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg text-lg font-semibold animate-slide-in">
                    ✅ Answer submitted!
                </div>
            )}
            ...
            <div className="w-full max-w-6xl mb-2 flex items-center justify-center gap-4">
                <div className="flex gap-1.5">{getTimerDots()}</div>
                <div className="flex gap-1.5">{getTimerDots()}</div>
            </div>
            ...
            <div className="w-full max-w-5xl mb-8 relative">
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 z-10">
                    <div className="relative w-32 h-20">
                        ...
                        <div className="absolute inset-0 bg-yellow-400 opacity-20 blur-xl" style={{ borderRadius: "100px 100px 0 0" }}></div>
                    </div>
                </div>
            ...
            <div className="mt-8 mb-4">
                <button
                    ...
                    className={`relative px-12 py-4 text-xl font-bold text-white transition-all duration-300 shadow-2xl ${
                        selectedOption !== null && !disableSubmitButton
                ...
                >
                    {disableSubmitButton ? "✓ Submitted" : selectedOption !== null ? "✓ SUBMIT ANSWER" : "SELECT AN OPTION"}
                </button>
            </div>
